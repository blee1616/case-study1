...
class StrategyBot(XChangeClient):
    ...
    def _write_final_pnl_log(self):
        ...
        for label in strategy_labels:
            ...
            # Debug check for attribution: any nonzero strategy positions at EOD.
            nonzero = {s: p for s, p in pos_map.items() if p != 0}
            if nonzero:
                print(f"[END POS {label}] {nonzero}")
    ...
    async def _force_unwind(self, symbol):
        ...
        # Attribute unwind fills to the originating directional strategy.
        if symbol in ("A", "ETF"):
            tag = STRAT_A_EARN
        elif symbol == "C":
            tag = STRAT_C_EARN
        ...
        if pos > 0:
            ...
            await self._place(symbol, qty, Side.SELL, px, strategy_label=tag)
        else:
            qty = min(-pos, chunk_cap, MAX_ORDER_SIZE)
            await self._place(symbol, qty, Side.BUY, px, strategy_label=tag)
        return False
