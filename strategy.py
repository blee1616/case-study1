...
# ---------- ETF arb params ----------
ETF_FEE = 2           # swap fee assumption (tune)
...
class StrategyBot(XChangeClient):
    ...
    def _init_logs(self):
        ...
        if not os.path.exists(PNL_LOG):
            with open(PNL_LOG, "w", newline="", encoding="utf-8") as pnl_file:
                ...
                pnl_writer.writerow(["symbol", "total_pnl"])
    ...
    def _write_final_pnl_log(self):
        ...
        for symbol in ALL_SYMBOLS:
            ...
            rows.append([symbol, round(total, 6)])

        rows.append(["TOTAL", round(portfolio_total, 6)])
    ...
    async def _enter_directional(self, symbol: str, direction: str, eps_delta: float):
        ...
        if direction == "LONG":
            ...
            burst = min(qty_needed, MAX_OUTSTANDING_VOL)
        ...
        if burst > 0:
            print(f"[DIRECTIONAL {symbol}] {direction} {burst} @ {px} (eps_delta={eps_delta:+.2f})")
            self._log_event(f"directional_{direction}", f"{symbol} qty={burst} eps_delta={eps_delta}")
            rem = burst
            while rem > 0:
                chunk = min(MAX_ORDER_SIZE, rem)
                await self._place(symbol, chunk, side, px)
                rem -= chunk
    ...
    async def _penny_requote(self, symbol, quote_size, soft_cap):
        ...
        if pos < soft_cap:
            self.my_quotes[symbol]["bid"] = await self._place(symbol, quote_size, Side.BUY, my_bid)
        if pos > -soft_cap:
            self.my_quotes[symbol]["ask"] = await self._place(symbol, quote_size, Side.SELL, my_ask)
    ...
    async def _etf_arb(self):
        ...
        if e_b > basket_cost + ETF_FEE + ETF_ARB_BUFFER:
            ...
            await self._place("ETF", ETF_ARB_SIZE, Side.SELL, e_b)
            await self._place("A", ETF_ARB_SIZE, Side.BUY, a_a)
            await self._place("B", ETF_ARB_SIZE, Side.BUY, b_a)
            await self._place("C", ETF_ARB_SIZE, Side.BUY, c_a)
            return
        ...
        if e_a < basket_bid - ETF_FEE - ETF_ARB_BUFFER:
            ...
            await self._place("ETF", ETF_ARB_SIZE, Side.BUY, e_a)
            await self._place("A", ETF_ARB_SIZE, Side.SELL, a_b)
            await self._place("B", ETF_ARB_SIZE, Side.SELL, b_b)
            await self._place("C", ETF_ARB_SIZE, Side.SELL, c_b)

    # ---------- Put-call parity arb on B options ----------
    async def _pcp_arb(self):
        # C - P = S - K  (r ~ 0, T ~ small)
        b_b, b_a = self._bbo("B")
        if b_b is None or b_a is None:
            return
        s_mid = (b_b + b_a) / 2
        for K in STRIKES:
            ...
            if None in (cb, ca, pb, pa):
                continue
            # synthetic long stock = buy call, sell put -> cost = ca - pb
            syn_long_cost = ca - pb + K
            # synthetic short stock proceeds = sell call, buy put -> proceeds = cb - pa
            syn_short_proceeds = cb - pa + K

            # If synthetic long < stock bid: buy synthetic, sell stock
            if syn_long_cost + PCP_BUFFER < b_b:
                print(f"[PCP {K}] buy call/sell put/sell B: syn={syn_long_cost} B_bid={b_b}")
                self._log_event("pcp_long_syn", f"K={K} edge={b_b-syn_long_cost}")
                await self._place(f"B_C_{K}", OPTION_ARB_SIZE, Side.BUY, ca)
                await self._place(f"B_P_{K}", OPTION_ARB_SIZE, Side.SELL, pb)
                await self._place("B", OPTION_ARB_SIZE, Side.SELL, b_b)
            # If synthetic short > stock ask: sell synthetic, buy stock
            elif syn_short_proceeds - PCP_BUFFER > b_a:
                print(f"[PCP {K}] sell call/buy put/buy B: syn={syn_short_proceeds} B_ask={b_a}")
                self._log_event("pcp_short_syn", f"K={K} edge={syn_short_proceeds-b_a}")
                await self._place(f"B_C_{K}", OPTION_ARB_SIZE, Side.SELL, cb)
                await self._place(f"B_P_{K}", OPTION_ARB_SIZE, Side.BUY, pa)
                await self._place("B", OPTION_ARB_SIZE, Side.BUY, b_a)
    ...
    async def _box_arb(self):
        ...
        if long_cost + BOX_BUFFER < target:
            ...
            await self._place(f"B_C_{K1}", OPTION_ARB_SIZE, Side.BUY, c1a)
            await self._place(f"B_C_{K2}", OPTION_ARB_SIZE, Side.SELL, c2b)
            await self._place(f"B_P_{K1}", OPTION_ARB_SIZE, Side.SELL, p1b)
            await self._place(f"B_P_{K2}", OPTION_ARB_SIZE, Side.BUY, p2a)
            return
        ...
        if short_proc - BOX_BUFFER > target:
            ...
            await self._place(f"B_C_{K1}", OPTION_ARB_SIZE, Side.SELL, c1b)
            await self._place(f"B_C_{K2}", OPTION_ARB_SIZE, Side.BUY, c2a)
            await self._place(f"B_P_{K1}", OPTION_ARB_SIZE, Side.BUY, p1a)
            await self._place(f"B_P_{K2}", OPTION_ARB_SIZE, Side.SELL, p2b)
            ...
            try:
                if self.phase == "PENNY":
                    ...
                    if elapsed >= HOLD_DURATION:
                        print(f"[STRATEGY] unwind -> PENNY, pos={self.positions}")
    ...
    async def bot_handle_news(self, news_release: dict):
        ...
        if kind == "structured" and subtype == "earnings":
            ...
            text = nd.get("content", "")
        self._all_news_writer.writerow([time.time(), kind, subtype, asset, symbol_field, text])
        self._all_news_file.flush()
    ...
    async def bot_handle_book_update(self, symbol):
        if self.phase == "PENNY" and symbol in ("A", "B", "C"):
            cap = SOFT_CAP[symbol]
